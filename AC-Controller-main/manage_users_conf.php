        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if ($row['add_card'] == 0) {
                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)) {
                    ...
                    $sql = "UPDATE users SET username=?, remaining_time=?, gender=?, email=?, user_date=CURDATE(), device_uid=?, device_dep=?, add_card=1 WHERE id=?";
                    $result = mysqli_stmt_init($conn);
                    if (!mysqli_stmt_prepare($result, $sql)) {
            ...
            if ($row['add_card'] == 0) {
                ...
                if (empty($Uname) && empty($Number) && empty($Email)) {
                    ...
                    if (!empty($Uname) && !empty($Email)) {
                        $sql = "UPDATE users SET username=?, remaining_time=?, gender=?, email=?, device_uid=?, device_dep=? WHERE id=?";
                        $result = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($result, $sql)) {
