                        ...
                        if ($row['add_card'] == 1) {
                            if ($row['device_uid'] == $device_uid || $row['device_uid'] == 0) {
                                ...
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    ...
                                    // Login
                                    if (!$row = mysqli_fetch_assoc($resultl)) {
                                        // Check if the serial number is greater than 0
                                        if ($Number <= 0) {
                                            echo "timeout"; // Serial number is 0 or negative, indicating timeout
                                            exit();
                                        }
                                        $sql = "INSERT INTO users_logs (username, remaining_time, card_uid, device_uid, device_dep, checkindate, timein, timeout) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                                        $result = mysqli_stmt_init($conn);
                                        if (!mysqli_stmt_prepare($result, $sql)) {
                                    ...
                                    // Logout
                                        ...
                                        $interval = $timein_datetime->diff($timeout_datetime);
                                        $seconds = ($interval->days * 24 * 60 * 60) + ($interval->h * 60 * 60) + ($interval->i * 60) + $interval->s;

                                        $sql = "UPDATE users_logs SET timeout=?, card_out=1, duration_seconds=? WHERE card_uid=? AND checkindate=? AND card_out=0";
                                        $result = mysqli_stmt_init($conn);
                                        if (!mysqli_stmt_prepare($result, $sql)) {
                                            echo "SQL_Error_insert_logout1";
                                            exit();
                                        } else {
                                            mysqli_stmt_bind_param($result, "ssis", $timeout, $seconds, $card_uid, $d);
                                            mysqli_stmt_execute($result);

                                            // Update the user's remaining_time
                                            ...
                                            if (!mysqli_stmt_prepare($result, $sql)) {
                                                ...
                                                mysqli_stmt_bind_param($result, "ds", $seconds, $card_uid);
                                                mysqli_stmt_execute($result);
